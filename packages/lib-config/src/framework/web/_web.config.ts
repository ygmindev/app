...
import type { WebConfigModel, _WebConfigModel } from '@lib/config/framework/web/_web.models';
...
export const _webConfig: _WebConfigModel = async () => {
  const { isSsr, publicDir } = await importConfig<WebConfigModel>('framework/web/web');
  const _bundleConfig = await importConfig<_BundleConfigModel>(
    'node/bundle/configs/bundle.config',
  );
