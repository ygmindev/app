...
import type { _WebConfigModel, WebConfigModel } from '@lib/config/platform/web/_web.models';
...
const _webConfig: _WebConfigModel = async () => {
  const { isSsr, publicDir } = await importConfig<WebConfigModel>('platform/web/web');
