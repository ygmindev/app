...
export class _Screen implements _ScreenModel {
  ...
  async open(uri: string): Promise<void> {
    console.warn('@@@ open test');
    await this.page.goto('http://bot.sannysoft.com/', {
  ...
  async snapshot({ filename }: { filename?: string }): Promise<Buffer | null> {
    await (this.page as unknown as Page).$eval('[banneroffsetheight="0"]', (h) => h?.remove());
    const element = await this.page.$('#bookingFunnelBody > div').then((h) => h?.boundingBox());
    const dirname = joinPaths([fromWorking(), this.options.snapshotPath]);
    !existsSync(dirname) && mkdirSync(dirname, { recursive: true });
    return (
      this.page.screenshot({
        clip: {
          height: element?.height ?? this.options.dimension.height ?? 1000,
          width: element?.width ?? this.options.dimension.width ?? 1000,
          x: 0,
          y: 0,
        },
