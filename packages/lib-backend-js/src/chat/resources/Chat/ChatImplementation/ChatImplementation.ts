...
import { UserFormModel, UserModel } from '@lib/shared/user/resources/User/User.models';

@withContainer({ name: `${CHAT_RESOURCE_NAME}Implementation` })
export class ChatImplementation
  extends createRelatedResourceImplementation<ChatModel, ChatFormModel, UserModel, UserFormModel>({
    Resource: Chat,
...
// @withContainer({ name: `${CHAT_RESOURCE_NAME}Implementation` })
// export class ChatImplementation
//   extends createProtectedResoureImplementation<ChatModel, ChatFormModel>({
//     Resource: Chat,
